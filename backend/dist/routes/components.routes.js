...
router.get("/", async (req, res) => {
    try {
        ...
        const components = await Component_1.Component.find(query).populate("authorId", "name email").sort({ createdAt: -1 });
        res.json(components);
    }
    catch (error) {
...
router.get("/me", auth_middleware_1.protect, async (req, res) => {
    try {
        const components = await Component_1.Component.find({ authorId: req.user._id }).sort({ createdAt: -1 });
        res.json(components);
    }
    catch (error) {
        res.status(500).json({ message: "Server error", error });
...
router.get("/id/:id", async (req, res) => {
    try {
        const component = await Component_1.Component.findById(req.params.id).populate("authorId", "name email");
        if (component) {
            res.json(component);
        }
        else {
...
router.get("/:slug", async (req, res) => {
    try {
        const component = await Component_1.Component.findOne({ slug: req.params.slug, status: "approved" }).populate("authorId", "name email");
        if (component) {
            res.json(component);
        }
        else {
